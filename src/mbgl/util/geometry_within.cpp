...
bool lineIntersectLine(const Point<int64_t>& a,
    ...
    const auto twoSided =
        [](const Point<int64_t>& p1, const Point<int64_t>& p2, const Point<int64_t>& q1, const Point<int64_t>& q2) {
            ...
            int64_t x1 = p1.x - q1.x;
            int64_t y1 = p1.y - q1.y;
            int64_t x2 = p2.x - q1.x;
            int64_t y2 = p2.y - q1.y;
            int64_t x3 = q2.x - q1.x;
            int64_t y3 = q2.y - q1.y;
...
template <typename T>
bool GeometryUtil<T>::segmentIntersectSegment(const Point<T>& a, const Point<T>& b,  const Point<T>& c, const Point<T>& d) const {
    // a, b are end points for line segment1, c and d are end points for line segment2
        ...
        // check if p1 and p2 are in different sides of line segment q1->q2
        const auto twoSided =
            [](const Point<T>& p1, const Point<T>& p2, const Point<T>& q1, const Point<T>& q2) {
                // q1->p1 (x1, y1), q1->p2 (x2, y2), q1->q2 (x3, y3)
                T x1 = p1.x - q1.x;
                T y1 = p1.y - q1.y;
                T x2 = p2.x - q1.x;
...
template<typename T>
bool GeometryUtil<T>::rayCast(const Point<T>& p, const Point<T>& p1, const Point<T>& p2) {
       return ((p1.y > p.y) != (p2.y > p.y)) && (p.x < (p2.x - p1.x) * (p.y - p1.y) / (p2.y - p1.y) + p1.x);
   }
...
template<typename T>
bool GeometryUtil<T>::pointWithinPolygon(const Point<T>& point, const Polygon<T>& polygon, bool checkOnBoundary) {
    bool within = false;
     for (const auto& ring : polygon) {
         const auto length = ring.size();
         // loop through every edge of the ring
         for (std::size_t i = 0; i < length - 1; ++i) {
             if (pointOnBoundary(point, ring[i], ring[i + 1])) return checkOnBoundary;
             if (rayCast(point, ring[i], ring[i + 1])) {
                 within = !within;
             }
         }
     }
...
//template <typename T>
//void GeoBBox<T>::updateBBox(BBox& bbox, Point<T> p) {
//       bbox[0] = std::min(p.x, bbox[0]);
//       bbox[1] = std::min(p.y, bbox[1]);
//       bbox[2] = std::max(p.x, bbox[2]);
//       bbox[3] = std::max(p.y, bbox[3]);
//}
//template <typename T>
//void GeoBBox<T>::getBBox(BBox& bbox, const MultiPoint<type>& points){
